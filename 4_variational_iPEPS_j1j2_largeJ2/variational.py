...
if __name__=='__main__':
    ...
    if args.model == 'J1J2':
        ...
        HnnAA   = args.J1*(2*kron(sz,sz)+(kron(sm,sp)+kron(sp,sm)))
        HnnAB   = args.J1*(2*kron(sz,4*sx@sz@sx)-(kron(sm, 4*sx@sp@sx)+kron(sp,4*sx@sm@sx)))
        HnnnAB  = args.J2*(2*kron(sz,4*sx@sz@sx)-(kron(sm, 4*sx@sp@sx)+kron(sp,4*sx@sm@sx)))
        H = torch.cat((HnnAA,HnnAB,HnnnAB))
